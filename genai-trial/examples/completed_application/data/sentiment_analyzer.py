...
# Map the output from the LLM (text) to a score
sentiment_map = {"POSITIVE": 1, "NEUTRAL": 0, "NEGATIVE": -1}

# On local machines, we might overload the LLM if we go too fast
# This adjusts the wait time between batches. Increase it if you
# detect issues due to the process being unable to keep up
wait_time = 0.5
...
def analyze_sentiment(text, product):
    ...
    try:
        ...
        # We must provide the prompt and model. 
        # We want a full response in one shot, not streamed output
        ...
        # Create the JSON node for the output, converting the text
        # output to a score using the map       
...
def process_batch(posts, batch_size=5):
    ...
    for i in range(0, len(posts), batch_size):
        ...
        for post in batch:
            ...
            # delay to handle slower, local processing
